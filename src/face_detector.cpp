...
bool FaceDetector::load(const std::filesystem::path& cascadePath) { return cascade_.load(cascadePath.string()); }

std::vector<cv::Rect> FaceDetector::detect(const cv::Mat& gray) {
    std::vector<cv::Rect> faces;
    cascade_.detectMultiScale(gray, faces, kScaleFactor, kMinNeighbors, cv::CASCADE_SCALE_IMAGE, kMinFaceSize);
    return faces;
}
